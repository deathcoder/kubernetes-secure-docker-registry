...
function configure_nodes {
  ...
  kubectl create configmap kube-registry-config --from-file=secrets/images/create_certs/in.req --from-file=secrets/images/create_certs/create_certs.sh --from-file=secrets/images/copy_certs/copy_certs.sh
  kubectl create -f secrets/k8s/create-certs.yaml
  ...
  tmp_file=$(mktemp)
  kubectl get nodes -o go-template-file --template ./secrets/k8s/copy-certs-templ.yaml > "$tmp_file"
  kubectl create -f "$tmp_file"
  rm "$tmp_file"

  echo
  echo "Removing any old registry and starting new one..."
  kubectl apply -f namespaces/docker-registry-namespace.yaml

  kubectl delete pvc --namespace=docker-registry private-registry-volume &> /dev/null || true
  kubectl create -f volumes/private-registry-persistentvolumeclaim.yaml
  kubectl delete deployment --namespace=docker-registry docker-registry
  kubectl create -f deployments/private-registry-deployment.yaml
  kubectl delete svc --namespace=docker-registry docker-registry &> /dev/null || true 
  kubectl create -f services/private-registry-service.yaml
  kubectl delete ingress --namespace=docker-registry docker-registry &> /dev/null || true
  kubectl create -f ingress/private-registry-ingress.yaml

  #wait for cert to become available
  echo
  echo -n "Waiting for cert to become available"
  set +e
  kubectl get --namespace=docker-registry secret ingress-registry-tls-secret &> /dev/null
  local rc=$?
  while [[ $rc != 0 ]]
  do
    sleep 1
    echo -n "."
    kubectl get --namespace=docker-registry secret registry-cert &>/dev/null
    rc=$?
  done
  set -e
}
...
will be stored as a Kubernetes secret named "registry-cert" in the docker-registry 
namespace.
...
function install_cert {
  ...
  if [[ -n "$file_path" && -n "$k8s_secret_set" ]]; then
    ...
    if [[ -z "$k8s_secret_ns" ]]; then
      #test if secret in default ns or docker-registry 
      set +e
      kubectl get secret "$k8s_secret" --namespace=docker-registry > /dev/null
      local rc=$?
      if [[ $rc == 0 ]]; then
        k8s_secret_ns="docker-registry"
      else
        ...
        if [[ $rc == 0 ]]; then
          ...
          echo "Cannot find secret $k8s_secret in docker-registry or default namespace"
          exit 1
        fi
      fi
      set -e
    ...
    echo "Retrieving certificate from Kubernetes secret $k8s_secret and namespace"
...
default_host="kube-registry.com"
